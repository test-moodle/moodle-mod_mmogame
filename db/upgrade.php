...
function xmldb_mmogame_upgrade( $oldversion) {
    ...
    if (count( $recs) == 0) {
        xmldb_mmogame_upgrade_import( 'mmogame_aa_colorpalettes');
    }

    $recs = $DB->get_records_select( 'mmogame_aa_avatars', '', null, '', '*', 0, 1);
    if (count( $recs) == 0) {
        xmldb_mmogame_upgrade_avatars();
    }

    return true;
}

function xmldb_mmogame_upgrade_import( $table) {
    global $DB;
...
function xmldb_mmogame_upgrade_avatars() {
    global $DB;
